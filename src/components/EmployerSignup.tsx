...
import { User, Mail, Lock, Eye, EyeOff, Upload, Camera, CheckCircle, AlertCircle, CreditCard } from 'lucide-react';
import { signUpEmployer, uploadFile, verifyEmployer } from '../lib/supabase';
...
const EmployerSignup: React.FC<EmployerSignupProps> = ({ onSignupComplete, onLoginClick }) => {
  ...
  const performFaceVerification = async () => {
    ...
    try {
      ...
      if (isMatch) {
        await verifyEmployer(userId, idDocumentUrl, selfieUrl);
  ...
  const handleStep1Submit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('Attempting to sign up employer with email:', formData.email);
      const result = await signUpEmployer(formData.email, formData.password, {
  ...
  const renderStep1 = () => (
    <form onSubmit={handleStep1Submit} className="space-y-6">
      ...
      <div>
        <label className="block text-white font-medium mb-2">Confirm Email *</label>
        <div className="relative">
          <Mail className="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-white/60" />
          <input
            type="email"
  ...
  const renderStep4 = () => (
    <div className="text-center space-y-6">
      ...
      <p className="text-white/80 text-lg">Your employer account has been verified and is ready to use.</p>
      ...
      <div className="bg-[#0FFCBE]/20 rounded-xl p-6">
        ...
        <div className="space-y-3 text-left">
          <div className="flex items-center space-x-3">
            ...
            <span className="text-white">Access to verified workers</span>
          </div>
          <div className="flex items-center space-x-3">
            <CheckCircle className="w-5 h-5 text-[#0FFCBE]" />
            <span className="text-white">Post job opportunities</span>
          </div>
          <div className="flex items-center space-x-3">
            <CheckCircle className="w-5 h-5 text-[#0FFCBE]" />
      ...
      <div className="max-w-md mx-auto">
        <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-8">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold text-white mb-2">Create Employer Account</h1>
            <p className="text-white/80">Join Path-Makers to find trusted workers</p>
          </div>
